...
function isNationaliteOrUrl($techno) {
    $techno = trim((string)($techno ?? ''));
    if (preg_match('/^(FR|UK|ES|IT|DE|NL|US|RU|CN|JP|IN|BR|PT|PL|BE|CA|CH|SE|NO|FI|DK|IE|AT|GR|TR|CZ|SK|HU|RO|BG|SI|HR|LT|LV|EE|LU|LI|MC|SM|VA|AD|IS|AL|BY|MD|UA|GE|AM|AZ|KZ|UZ|TM|KG|TJ|MN|RS|ME|MK|BA|XK)$/i', $techno)) {
        return true;
    }
...
if (isset($client['id'])) {
    ...
    if (isset($client['id'])) {
        ...
        // Trouver le dernier scan_id de amass_scan_tmp pour ce client
        $scanIdReq = $pdo2->prepare("SELECT id FROM amass_scan_tmp WHERE client_id = :client_id ORDER BY start_time DESC LIMIT 1");
        $scanIdReq->execute(['client_id' => $client['id']]);
        $last_scan_id = $scanIdReq->fetchColumn();
        if ($last_scan_id) {
            // On suppose que la table amass_result a un champ scan_id qui fait référence à amass_scan_tmp.id
            $stmt = $pdo2->prepare("SELECT * FROM amass_result WHERE scan_id = :scan_id");
            $stmt->execute(['scan_id' => $last_scan_id]);
            $amass_structured_results = $stmt->fetchAll(PDO::FETCH_ASSOC);
        }
    }

// --- Extraction et affichage des résultats Amass sous forme de tableau ---
$amass_results = [];
if (!empty($amass_debug_output)) {
    $lines = explode("\n", $amass_debug_output);
    foreach ($lines as $line) {
        $line = trim($line);
        if ($line === '' || strpos($line, "Scan Amass en cours") !== false) continue;
        // Pattern général : source --> relation --> destination (type)
...
<head>
    ...
    <style>
        ...
        .amass-table { border-collapse: collapse; margin-top:1em; background:#fff; }
        .amass-table th, .amass-table td { border:1px solid #bbb; padding: 0.25em 0.6em; font-size:0.98em; }
        .amass-table th { background: #f3f3f3; }
        .amass-table tr:hover { background: #eefcdf; }
    </style>
</head>
<body>
...
<main>
    ...
    <?php if ($amass_debug_output !== null): ?>
        <div class="amass-debug-output">
            ...
            <details>
                <summary>Afficher/masquer la table des résultats Amass</summary>
                <?php if (!empty($amass_results)): ?>
                    <table class="amass-table">
                        <thead>
                            <tr>
                                <th>Source</th>
                                <th>Type source</th>
                        ...
                        <tbody>
                        <?php foreach ($amass_results as $row): ?>
                            <tr>
                                <td><?= htmlspecialchars($row['source']) ?></td>
                                <td><?= htmlspecialchars($row['source_type']) ?></td>
                                <td><?= htmlspecialchars($row['relation']) ?></td>
                                <td><?= htmlspecialchars($row['destination']) ?></td>
                                <td><?= htmlspecialchars($row['destination_type']) ?></td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
                    </table>
                <?php else: ?>
                    <i>Aucun résultat structuré parsé (brut affiché ci-dessous).</i>
                <?php endif; ?>
            </details>
            <br>
            <b>Sortie brute :</b><br>
    ...
    <!-- ... Whatweb results table inchangé ... -->
    <!-- ... (gardez la partie whatweb telle qu'elle était dans le fichier original) ... -->
    <h2>Résultats Amass structurés (dernier scan)</h2>
    <?php if (!empty($amass_structured_results)): ?>
        <table class="amass-table">
            <thead>
                <tr>
                    <th>Left</th>
            ...
            <tbody>
                <?php foreach ($amass_structured_results as $row): ?>
                    <tr>
                        <td><?= htmlspecialchars($row['left']) ?></td>
                        <td><?= htmlspecialchars($row['left_type']) ?></td>
                        <td><?= htmlspecialchars($row['relation']) ?></td>
                        <td><?= htmlspecialchars($row['right']) ?></td>
                        <td><?= htmlspecialchars($row['right_type']) ?></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    <?php else: ?>
        <div style="color:#888; margin-bottom:1em;">Aucun résultat structuré Amass pour ce client.</div>
    <?php endif; ?>

    <!-- Affichage de la sortie brute pour debug -->
    <?php if ($amass_debug_output !== null): ?>
        <div class="amass-debug-output">
            <b>Sortie brute Amass :</b><br>
            <?= nl2br(htmlspecialchars($amass_debug_output)) ?>
