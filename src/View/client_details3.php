...
// --- Suppression des résultats Amass (inchangé) ---
...
// --- AFFICHE LE DERNIER RESULTAT AMASS, PAS D'EXECUTION ---
$amass_debug_output = null;
$amass_scan_finished = false;
if (isset($client['id'])) {
    $pdo = new PDO(
        "pgsql:host={$config['db']['host']};port={$config['db']['port']};dbname={$config['db']['dbname']}",
        $config['db']['user'],
...
// --- Whatweb : PAS D'EXECUTION, juste lecture en base ---
$pdo = new PDO(
    "pgsql:host={$config['db']['host']};port={$config['db']['port']};dbname={$config['db']['dbname']}",
    $config['db']['user'],
    $config['db']['pass']
...
<form method="post" action="/asd001/public/api/launch_amass.php" style="display:inline;">
    <input type="hidden" name="client_id" value="<?= $client['id'] ?>">
    <input type="hidden" name="domain" value="<?= htmlspecialchars($assets[0]['value'] ?? '') ?>">
    <button type="submit" style="background:#093;color:white;">Lancer un scan Amass maintenant</button>
</form>

<!-- Bouton lancer un scan Whatweb maintenant -->
<form method="post" action="/asd001/public/api/launch_whatweb.php" style="display:inline;">
    <input type="hidden" name="client_id" value="<?= $client['id'] ?>">
    <input type="hidden" name="domain" value="<?= htmlspecialchars($assets[0]['value'] ?? '') ?>">
    <button type="submit" style="background:#1c6ca1;color:white;">Lancer un scan Whatweb maintenant</button>
</form>
